...
function humanizeMode(mode: string) {
  const map: Record<string, string> = { heuristic: "Heurístico", hybrid: "Híbrido" };
...
function getRecommendationWarnings(item: TaskRecommendationItem | CandidateBucketItem) {
  ...
  if (item.matching_skills.length === 0 || skillScore <= 0) warnings.push("No registra habilidades coincidentes con la tarea.");
  if (availability <= 0) warnings.push("No tiene disponibilidad operativa inmediata.");
  if (currentLoad >= 95) warnings.push("Tiene una carga actual muy alta.");
  if (item.risk_level === "high") warnings.push("La asignación tiene riesgo alto y conviene revisarla.");
  if (item.model_used && mlProbability < 0.3) warnings.push("La probabilidad del modelo es baja para esta asignación.");
  if ("assignability_status" in item && item.assignability_status === "not_assignable") warnings.push("La IA no recomienda asignarlo ahora.");
...
function getDecisionBannerClasses(data: TaskRecommendationResponse | null) {
  ...
  if (data.primary_candidate_available) return "border-green-500/20 bg-green-500/10 text-green-300";
...
function getActionGuidance(action?: RecommendedAction | string | null) {
  ...
  return map[action || ""] ?? { title: "Sugerencia ejecutiva", bullets: ["Revisa la decisión con base en capacidad operativa, skills y riesgo."] };
}
...
function ManualOverrideModal({
  ...
  candidate: CandidateBucketItem | null;
  note: string;
  accepted: boolean;
  ...
  const warnings = getRecommendationWarnings(candidate);

  return (
    <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="w-full max-w-2xl rounded-2xl border border-yellow-500/20 bg-[#081225] p-6 space-y-5">
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            <TriangleAlert className="w-6 h-6 text-yellow-400" />
            <div>
              <h2 className="text-2xl text-white">Asignación manual bajo aceptación de riesgo</h2>
              <p className="text-slate-400 text-sm">
                Estás por asignar manualmente a <span className="text-white font-semibold">{candidate.member.full_name}</span>.
              </p>
            </div>
          </div>
          <button onClick={onClose} className="rounded-lg border border-slate-700 bg-slate-900 p-2 text-slate-300 hover:bg-slate-800">
            <X className="w-4 h-4" />
          </button>
        </div>

        <div className="rounded-xl border border-yellow-500/20 bg-yellow-500/10 p-4">
          <p className="text-yellow-300 font-medium mb-2">La IA no recomienda esta asignación</p>
          <div className="space-y-1">
            {warnings.map((warning) => (
              <p key={warning} className="text-yellow-100 text-sm">• {warning}</p>
            ))}
          </div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
          <MetricPill label="Score" value={candidate.score.toFixed(2)} />
          <MetricPill label="Disponibilidad" value={formatPercentNumber(candidate.availability)} />
          <MetricPill label="Carga actual" value={formatPercentNumber(candidate.current_load)} />
          <MetricPill label="Probabilidad ML" value={candidate.model_used ? formatPercentNumber((candidate.ml_success_probability ?? 0) * 100) : "No aplicada"} />
        </div>

        <div>
          <label className="block text-slate-300 text-sm mb-2">Justificación manual del líder</label>
          <textarea value={note} onChange={(e) => onChangeNote(e.target.value)} rows={4} className="w-full rounded-xl bg-slate-900 border border-slate-700 px-4 py-3 text-white resize-none" placeholder="Ejemplo: Se asigna por urgencia operativa, conocimiento del módulo y coordinación directa del líder." />
        </div>

        <label className="flex items-start gap-3 rounded-xl border border-slate-800 bg-slate-900/60 p-4 cursor-pointer">
          <input type="checkbox" checked={accepted} onChange={(e) => onToggleAccepted(e.target.checked)} className="mt-1" />
          <span className="text-slate-300 text-sm">
            Confirmo que esta será una <span className="text-white font-semibold">asignación manual bajo aceptación explícita de riesgo</span>, y que no corresponde a una recomendación segura de la IA.
          </span>
        </label>
        ...
        <div className="flex justify-end gap-3">
          ...
          <button onClick={onConfirm} disabled={!accepted || !note.trim() || loading} className={`px-4 py-2 rounded-lg transition-all ${ !accepted || !note.trim() || loading ? "bg-slate-800 text-slate-500 cursor-not-allowed" : "bg-yellow-400 text-slate-950 hover:bg-yellow-300" }`} >
            {loading ? "Asignando..." : "Confirmar asignación manual"}
          </button>
        </div>
      </div>
    </div>
...
function CandidateCard({
  ...
  manualOverrideLabel = "Asignar bajo riesgo",
  emphasizeNotAssignable = false,
}: {
  item: CandidateBucketItem;
  onAssign: (memberId: number) => Promise<void>;
  onManualOverride: (candidate: CandidateBucketItem) => void;
  assigning: boolean;
  allowAssign: boolean;
    ...
    <div className="rounded-xl border border-slate-800 bg-slate-950/40 p-5 space-y-4">
      ...
      {warnings.length > 0 && (
        <div className="rounded-lg border border-yellow-500/20 bg-yellow-500/10 p-3">
          ...
          <div className="space-y-1">
            {warnings.map((warning) => (
              <p key={warning} className="text-yellow-200 text-sm">• {warning}</p>
            ))}
          </div>
        </div>
      )}
      ...
      <div className="flex justify-end gap-3 flex-wrap">
        {allowManualOverride && (
          <button onClick={() => onManualOverride(item)} disabled={assigning} className="inline-flex items-center gap-2 px-4 py-2 rounded-lg border border-yellow-500/20 bg-yellow-500/10 text-yellow-300 hover:bg-yellow-500/15 transition-all" >
...
export default function SmartRecommendation() {
  ...
  const [heuristicRecommendations, setHeuristicRecommendations] = useState<TaskRecommendationResponse | null>(null);
  const [hybridRecommendations, setHybridRecommendations] = useState<TaskRecommendationResponse | null>(null);
  ...
  const [manualOverrideCandidate, setManualOverrideCandidate] = useState<CandidateBucketItem | null>(null);
  ...
  const activeRecommendations = useMemo(() => (
    mode === "heuristic" ? heuristicRecommendations : hybridRecommendations
  ...
  const sameTopCandidate =
    heuristicTop && hybridTop && heuristicTop.member.id === hybridTop.member.id;
  ...
  const canUseManualOverride = ["admin", "leader"].includes(currentRole);
  ...
  const openManualOverride = (candidate: CandidateBucketItem) => {
    setManualOverrideCandidate(candidate);
    setManualOverrideAccepted(false);
    setManualOverrideNote("");
  };

  const closeManualOverride = () => {
    setManualOverrideCandidate(null);
    setManualOverrideAccepted(false);
    setManualOverrideNote("");
  };

  const confirmManualOverride = async () => {
    if (!taskId || !token || !manualOverrideCandidate || !manualOverrideNote.trim()) return;

    try {
      setAssigningMemberId(manualOverrideCandidate.member.id);
      setError("");
      setSuccessMessage("");
      ...
      await assignTaskFromRecommendation(
        taskId,
        {
          assigned_to: manualOverrideCandidate.member.id,
          assigned_by: currentUser?.id ?? null,
          source: `${mode}_manual_override`,
          strategy,
          recommendation_score: manualOverrideCandidate.score,
          risk_level: manualOverrideCandidate.risk_level,
          reason: `[ASIGNACIÓN MANUAL BAJO RIESGO] ${manualOverrideNote.trim()} | Base IA: ${manualOverrideCandidate.reason}`,
          recommendation_used: false,
        },
        token
      );

      setSuccessMessage("La tarea fue asignada manualmente bajo aceptación de riesgo.");
      closeManualOverride();
      navigate(`/task/${taskId}`);
    } catch (err) {
      if (err instanceof Error) setError(err.message);
      else setError("No se pudo registrar la asignación manual.");
    } finally {
      setAssigningMemberId(null);
    }
  };
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      <ManualOverrideModal candidate={manualOverrideCandidate} note={manualOverrideNote} accepted={manualOverrideAccepted} onChangeNote={setManualOverrideNote} onToggleAccepted={setManualOverrideAccepted} onClose={closeManualOverride} onConfirm={confirmManualOverride} loading={assigningMemberId === manualOverrideCandidate?.member.id} />
      ...
      {successMessage && <div className="rounded-xl border border-green-500/30 bg-green-500/10 px-4 py-3 text-green-300 text-sm">{successMessage}</div>}
      ...
      <div className={`rounded-2xl border p-6 ${getDecisionBannerClasses(activeRecommendations)}`}>
        <div className="flex items-start gap-3">
          {activeRecommendations?.primary_candidate_available ? <CheckCircle2 className="w-6 h-6 mt-0.5" /> : <AlertTriangle className="w-6 h-6 mt-0.5" />}
          <div className="space-y-2">
            <h2 className="text-xl font-semibold">{humanizeDecisionStatus(activeRecommendations?.decision_status)}</h2>
            <p className="text-sm opacity-90">
              Acción recomendada por la IA: <span className="font-semibold">{humanizeRecommendedAction(activeRecommendations?.recommended_action)}</span>
            </p>
            {activeRecommendations && (
              <div className="flex flex-wrap gap-3 text-sm">
                <span>Asignables: <strong>{activeRecommendations.assignability_summary.assignable_count}</strong></span>
                <span>Con riesgo: <strong>{activeRecommendations.assignability_summary.risky_count}</strong></span>
                <span>No asignables: <strong>{activeRecommendations.assignability_summary.not_assignable_count}</strong></span>
              </div>
            )}
          </div>
        </div>
      ...
      {!primaryCandidate && (
        <>
          <div className="rounded-2xl border border-cyan-500/20 bg-cyan-500/10 p-6">
            ...
            <div className="space-y-2">
              {actionGuidance.bullets.map((bullet) => <p key={bullet} className="text-cyan-100 text-sm">• {bullet}</p>)}
            </div>
          </div>

          {canUseManualOverride && (
            <div className="rounded-2xl border border-yellow-500/20 bg-yellow-500/10 p-6">
              <div className="flex items-center gap-3 mb-3">
                ...
                <h2 className="text-xl text-yellow-300">Desbloqueo controlado para líder/admin</h2>
              </div>
              <p className="text-yellow-100 text-sm leading-relaxed">
                Si necesitas avanzar pese al bloqueo de la IA, puedes hacer una <span className="font-semibold">asignación manual bajo aceptación explícita de riesgo</span>. El sistema dejará trazabilidad de que no fue una recomendación segura de la IA.
              </p>
            </div>
          )}
        </>
      ...
      {primaryCandidate ? (
        <div className="rounded-2xl border border-slate-800 bg-slate-900 p-6 space-y-4">
          ...
          <CandidateCard item={primaryCandidate} onAssign={handleAssign} onManualOverride={openManualOverride} assigning={assigningMemberId === primaryCandidate.member.id} allowAssign={Boolean(activeRecommendations?.primary_candidate_available)} />
        </div>
      ) : (
        <div className="rounded-2xl border border-yellow-500/20 bg-yellow-500/10 p-6">
          <div className="flex items-center gap-3 mb-3">
          ...
          <p className="text-yellow-100 text-sm leading-relaxed">
            La IA no encontró un candidato que cumpla simultáneamente con la capacidad operativa y el mínimo técnico esperado. La acción sugerida es <span className="font-semibold">{humanizeRecommendedAction(activeRecommendations?.recommended_action)}</span>.
          </p>
        </div>
      )}
      ...
      {topReferenceCandidate && (
        <div className="rounded-2xl border border-slate-800 bg-slate-900 p-6">
          ...
          <p className="text-slate-400 text-sm mb-4">
            Este perfil aparece primero en el ranking actual, pero la IA no lo considera asignable para esta tarea.
          </p>
          <div className="rounded-xl border border-slate-800 bg-slate-950/40 p-5">
            <div className="flex items-start justify-between gap-4 flex-wrap mb-3">
              ...
              <div className="flex gap-2 flex-wrap">
                <span className="px-3 py-1 rounded-lg border border-yellow-500/20 bg-yellow-500/10 text-yellow-300 text-xs">Solo referencia</span>
                <span className="px-3 py-1 rounded-lg border border-slate-700 bg-slate-900 text-slate-300 text-xs">Score {topReferenceCandidate.score.toFixed(2)}</span>
              </div>
            </div>
        ...
        <div className="rounded-2xl border border-slate-800 bg-slate-900 p-6">
          ...
          <div className="flex flex-wrap gap-2 mb-3">
            {insights.suggested_skills.map((skill) => (
              <span key={skill} className="px-3 py-2 rounded-lg border border-cyan-500/20 bg-cyan-500/10 text-cyan-300 text-sm">{skill}</span>
            ))}
          </div>

          <div className="space-y-1">
            {insights.detected_signals.map((signal) => <p key={signal} className="text-slate-400 text-sm">• {signal}</p>)}
          </div>
        </div>
      )}
            ...
            <div key={kind} className={`rounded-2xl border p-6 space-y-4 ${config.wrapper}`}>
              ...
              {items.length === 0 ? (
                ...
                <div className="space-y-4">
                  {items.map((item) => (
                    <CandidateCard key={`${kind}-${item.member.id}`} item={item} onAssign={handleAssign} onManualOverride={openManualOverride} assigning={assigningMemberId === item.member.id} allowAssign={kind === "assignable" && Boolean(activeRecommendations?.primary_candidate_available)} allowManualOverride={(kind === "risky" || kind === "not_assignable") && canUseManualOverride} manualOverrideLabel={kind === "risky" ? "Asignar con riesgo" : "Asignar bajo riesgo"} emphasizeNotAssignable={kind === "not_assignable"} />
                  ))}
                </div>
              )}
            </div>
      ...
      <div className="rounded-2xl border border-slate-800 bg-slate-900 p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h2 className="text-2xl text-white">
            {activeRecommendations?.primary_candidate_available ? "Comparación Heurístico vs Híbrido" : "Comparación de perfiles de referencia"}
          </h2>
        </div>

        {!activeRecommendations?.primary_candidate_available && (
          <p className="text-slate-400 text-sm mb-4">
            Los perfiles mostrados abajo son referencias de ranking. No representan candidatos asignables para esta tarea en el estado actual.
          </p>
        )}

        <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
          <div className="rounded-xl border border-slate-800 bg-slate-950/40 p-5">
            <p className="text-slate-400 text-xs mb-2">{activeRecommendations?.primary_candidate_available ? "Top Heurístico" : "Referencia Heurística"}</p>
            <p className="text-white text-lg font-semibold">{heuristicTop?.member.full_name || "—"}</p>
            <p className="text-slate-400 text-sm mt-1">Puntaje: {heuristicTop?.score?.toFixed(2) || "—"}</p>
            <p className="text-slate-400 text-sm">Riesgo: {heuristicTop ? humanizeRisk(heuristicTop.risk_level) : "—"}</p>
          </div>

          <div className="rounded-xl border border-slate-800 bg-slate-950/40 p-5">
            <p className="text-slate-400 text-xs mb-2">{activeRecommendations?.primary_candidate_available ? "Top Híbrido" : "Referencia Híbrida"}</p>
            <p className="text-white text-lg font-semibold">{hybridTop?.member.full_name || "—"}</p>
            <p className="text-slate-400 text-sm mt-1">Puntaje: {hybridTop?.score?.toFixed(2) || "—"}</p>
            <p className="text-slate-400 text-sm">Riesgo: {hybridTop ? humanizeRisk(hybridTop.risk_level) : "—"}</p>
          </div>

          <div className="rounded-xl border border-slate-800 bg-slate-950/40 p-5">
            <p className="text-slate-400 text-xs mb-2">Lectura rápida</p>
            <p className="text-white text-lg font-semibold">{sameTopCandidate ? "Coinciden" : "Cambian de candidato"}</p>
          ...
          <div className="space-y-4">
            {simulation.simulations.map((item) => (
              <div key={item.member.id} className="rounded-xl border border-slate-800 bg-slate-950/40 p-5 space-y-4">
                <div className="flex items-start justify-between gap-4 flex-wrap">
                  <div>
                    <p className="text-white text-lg font-semibold">#{item.rank} · {item.member.full_name}</p>
      ...
      <div className="rounded-2xl border border-slate-800 bg-slate-900 p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h2 className="text-2xl text-white">{activeRecommendations?.primary_candidate_available ? "Ranking resumido" : "Perfiles evaluados por la IA"}</h2>
        </div>

        {!activeRecommendations?.primary_candidate_available && (
          <p className="text-slate-400 text-sm mb-4">
            El ranking se muestra para transparencia analítica, pero ninguno de estos perfiles supera el filtro final de asignación.
          </p>
        )}
              ...
              <div className="flex items-start justify-between gap-4 flex-wrap mb-3">
                <div>
                  <div className="flex items-center gap-2 flex-wrap">
                    <p className="text-white text-lg font-semibold">{item.member.full_name}</p>
                    {!activeRecommendations?.primary_candidate_available && (
                      <span className="px-3 py-1 rounded-lg border border-red-500/20 bg-red-500/10 text-red-300 text-xs">No asignable</span>
                    )}
                  </div>
                  <p className="text-slate-400 text-sm">{item.member.email}</p>
                </div>
                ...
                <div className="flex gap-2 flex-wrap">
                  <span className="px-3 py-1 rounded-lg border border-slate-700 bg-slate-900 text-slate-300 text-xs">Score {item.score.toFixed(2)}</span>
                  <span className="px-3 py-1 rounded-lg border border-slate-700 bg-slate-900 text-slate-300 text-xs">Riesgo {humanizeRisk(item.risk_level)}</span>
                </div>
              </div>
