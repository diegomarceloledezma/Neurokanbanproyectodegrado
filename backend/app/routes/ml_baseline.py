...
def baseline_status(current_user: User = Depends(get_current_user)):
    if not has_any_role(current_user, "admin", "leader"):
        raise HTTPException(
            status_code=403,
            detail="No tienes permisos para ver el estado del baseline",
        )
...
def train_baseline_from_history(
    ...
    if not has_any_role(current_user, "admin", "leader"):
        raise HTTPException(
            status_code=403,
            detail="No tienes permisos para entrenar el baseline",
        )
...
def train_baseline_from_history_cleaned(
    ...
    if not has_any_role(current_user, "admin", "leader"):
        raise HTTPException(
            status_code=403,
            detail="No tienes permisos para entrenar el baseline limpio",
        )
...
def train_baseline_from_history_compact_cleaned(
    ...
    if not has_any_role(current_user, "admin", "leader"):
        raise HTTPException(
            status_code=403,
            detail="No tienes permisos para entrenar el baseline compacto",
        )
